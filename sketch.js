...
function generateQuestions() {
    ...
    const unitFactors = [1, 10, 100, 1000, 10000, 100000, 1000000];

    function convert(value, fromIdx, toIdx) {
        let inMM = value * unitFactors[fromIdx];
        return inMM / unitFactors[toIdx];
    }

    let val1 = Math.floor(random(1, 101)) * 10;
    let val2;
    do {
        val2 = Math.floor(random(1, 101)) * 10;
    } while (val2 === val1);

    let questions = [];
    let usedQuestions = new Set();
    // ← NIEUW: bijhoud hoeveel keer elk antwoord al voorkomt
    let answerCount = {};

    function tryAddQuestion(val, targetLength) {
        let attempts = 0;
        while (questions.length < targetLength && attempts < 500) {
            attempts++;
            let fromIdx = Math.floor(random(units.length));
            let steps = Math.floor(random(1, 4));
            let dir = random() < 0.5 ? 1 : -1;
            let toIdx = fromIdx + steps * dir;
            if (toIdx < 0 || toIdx >= units.length) continue;

            let key = val + '_' + fromIdx + '_' + toIdx;
            if (usedQuestions.has(key)) continue;

            let answer = convert(val, fromIdx, toIdx);
            if (answer !== Math.floor(answer)) continue;
            if (answer <= 0 || answer > 100000) continue;

            // ← NIEUW: blokkeer als dit antwoord al 2x voorkomt
            let currentCount = answerCount[answer] || 0;
            if (currentCount >= 2) continue;

            usedQuestions.add(key);
            answerCount[answer] = currentCount + 1;
            questions.push({
                text: val + ' ' + units[fromIdx] + '\n= ? ' + units[toIdx],
                answer: answer
            });
        }
    }

    tryAddQuestion(val1, 5);
    tryAddQuestion(val2, 10);
