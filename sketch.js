...
function generateQuestions() {
    ...
    let val1 = Math.floor(random(1, 101)) * 10; // 10, 20, ..., 1000
let val2;
do {
    ...
    // Genereer 5 vragen met val1 als startgetal
    let attemptsA = 0;
    while (questions.length < 5 && attemptsA < 200) {
        attemptsA++;
        ...
        let key = val1 + '_' + fromIdx + '_' + toIdx;
        if (usedQuestions.has(key)) continue;

        let answer = convert(val1, fromIdx, toIdx);
        if (answer !== Math.floor(answer)) continue;
        if (answer <= 0 || answer > 100000) continue;

        usedQuestions.add(key);
        questions.push({
            text: val1 + ' ' + units[fromIdx] + '\n= ? ' + units[toIdx],
            answer: answer
        });
    }

    // Genereer 5 vragen met val2 als startgetal
    let attemptsB = 0;
    while (questions.length < 10 && attemptsB < 200) {
        attemptsB++;
        let fromIdx = Math.floor(random(units.length));
        let steps = Math.floor(random(1, 4));
        let dir = random() < 0.5 ? 1 : -1;
        ...
        let key = val2 + '_' + fromIdx + '_' + toIdx;
        if (usedQuestions.has(key)) continue;

        let answer = convert(val2, fromIdx, toIdx);
        if (answer !== Math.floor(answer)) continue;
        if (answer <= 0 || answer > 100000) continue;

        usedQuestions.add(key);
        questions.push({
            text: val2 + ' ' + units[fromIdx] + '\n= ? ' + units[toIdx],
            answer: answer
        });
    }
